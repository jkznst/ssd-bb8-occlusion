...
def train_net(network, train_path, num_classes, batch_size,
    ...
    mod = mx.mod.Module(net, data_names=data_names, label_names=label_names, logger=logger, context=ctx,
    ...
    if tensorboard:
        ...
        eval_end_callback.append(LogMetricsCallback(
            os.path.join(tensorboard_dir, 'val/scalar'), 'ssd', global_step=0))
    ...
    logger.info("learning rate: {}, lr refactor step: {}, lr refactor ratio: {}, batch size: {}.".format(
        learning_rate, lr_refactor_step, lr_refactor_ratio, batch_size))
    ...
    mod.fit(train_iter,
            ...
            validation_metric=MultiBoxMetric(),    # use 'valid_metric' for calculate mAP
